...
+ `vImage_CGImageFormat`: 基本vImage同一接口，会提供不同色彩格式的类似借口，比如ARGB8888，指的是ARGB排列，每通道占8个Bit，也就是一个Piexel占32Bit。而vImage还有一个常见的色彩格式Plane8
+ `vImage_Flags`: 不同的vImage接口，有特殊的选项来控制，比如说内存分配，填充策略，重采样策略等，默认的是`kvImageNoFlags`
+ `vImage_Error`: 每个vImage的接口，都会返回这个result，来让用户确认是否成功，以及失败的原因，在Debug下比较有帮助

为了统一期间，以下的内容，都是基于ARGB8888色彩格式的输入来说明的。其他的情况处理，参考同名方法的不同格式即可。

# 色彩转换

色彩转换指的是将图像的Bitmap格式，从一个色彩格式，比如ARGB8888，转换到另一个色彩格式，比如说RGB888的功能。对于RGB的，一般来说就是通道的增加和减少。当然还有RGB转为Planar8的情况。
...
除了一系列`vImageConvert_AtoB`的转换，vImage还提供了一个非常抽象的接口，叫做`vImageConvert_AnyToAny`，只需要你提供一个input format，一个output format。这个接口比较强大，不仅能够handler所有支持的色彩格式，而且还能支持`CVImageBuffer`（通过这个`vImageConverter`来构造）。所以一般如果做库封装，做一些色彩转换的case的时候，就可以试着用这个接口。
...
[Alpha合成](https://en.wikipedia.org/wiki/Alpha_compositing)指的是将两张含有Alpha通道的图，通过一定的公式合成成为一张新的含Alpha通道的图，一般来说用于给图像添加遮罩、覆盖等。其实本质上来说，Alpha合成，就是对图像的每一个像素值，进行这样一个计算：
...
```objectivec
    ...
    __block vImage_Buffer a_buffer = {}, b_buffer = {}, output_buffer = {}; // 分别是Top Buffer，Bottom buffer和最后的输出的Buffer
...
```objectivec
    CGImageRef aImage, bImage; // 输入的Bottom Image和Mask Image
    __block vImage_Buffer a_buffer = {}, b_buffer = {}, c_buffer = {}, output_buffer = {};
    //C Buffer指的是将Mask Image进行处理后的临时Buffer，使得宽高同Bottom Image相同
    // 这里我们使用到了线性变换的平移变换，以(0,0)放置Mask Image，然后偏移Point个像素点，其余部分填充Clear Color，即可得到这个处理后的Mask Image
    ...
    // 略过output buffer初始化
    // 将Bottom Image和处理后的Mask Image进行Alpha Blend
...
以下的这些几何变换，其实都最后可以转换到线性变换上，只不过效率上可能有所损耗，因此单独对每个功能提供的vImage接口进行了介绍。关于线性变换不太理解的，可以参考一下之前的一篇教程：[Core Graphics仿射变换知识](http://dreampiggy.com/2016/09/27/core-graphicsfang-she-bian-huan/)
...
在vImage总，错切变换是相对底层的接口，实际上，线性变换是通过这三个接口（错切、旋转、镜像来实现的）。错切的接口对应的是`vImageHorizontalShear_ARGB8888`，参数算是最多的一个，稍微详细介绍一下：

+ `srcOffsetToROI_X`: 错切定位点水平偏移量，具体指的就是左上角那个像素点，在经过旋转的映射后，水平偏移的距离，会影响最后图像（除去Buffer的宽度限制）的整体宽度和高度
...
最后再来说通用的线性变换吧，这个其实在之前的功能中已经用到过了，vImage可以有接口接受Core Graphics的`CGAffineTransform`结构体的接口，结构体对应的内存布局是一样的，直接强制转换过去就可以了，不需要单独额外处理。关于通用线性变换的内容就不再赘述了，有兴趣可以查看相关资料，或者之前的教程：[Core Graphics仿射变换知识](http://dreampiggy.com/2016/09/27/core-graphicsfang-she-bian-huan/)
...
vImage是一个比较底层的图像Bitmap处理的库，在这里介绍了关于Alpha合成、几何变换等基本知识。相比于简单的Core Graphics的处理，能够提供更为深入的参数控制，并且带来较高的性能。

但是vImage强大之处远不在这里，里面包含了类似图像卷积，形态处理等，可以对复杂滤镜进行支持，这些都需要数字图像处理相关知识，在这种教程系列就不会介绍了。
...
由于自己完全是业务兴趣，工作和图像处理基本不相关，并不打算深入学习数字图像处理的知识，因此这个教程可能就会暂时告一段落了。最后，希望这系列教程，能给同样对图像边解码、图像处理有一点兴趣的人，提供一个相对简单的入门概览吧。