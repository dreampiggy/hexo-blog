...
ReactiveCocoa把事件流的接口，定义为`RACStream`。而实际上，通常的事件流实现都是`RACSignal`对象，这个Signal是一个冷事件流（也可以叫做push-driven），即有订阅者订阅后，才会`开始从头依次发送事件`。而对应的冷事件流接口叫做`RACMulticastConnection`，即没有订阅者也会发送事件流。冷事件可以通过`publish`和`multicast`转换到热事件流，这对于很多请求，比如WebSocket这种不需要重入的事件流来说很有用。
